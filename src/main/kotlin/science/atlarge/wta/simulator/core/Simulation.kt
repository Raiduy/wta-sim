...
class Simulation(
    ...
    private val MILLIS_BETWEEN_ENV_EXPOSE = 900000  // get environment status every 15 minutes
    ...
    fun simulate() {
        ...
        var exposeSubmissionTime = trace.tasks.first().submissionTime
        eventQueue.submit(ExposeEnvironmentEvent(exposeSubmissionTime, environment))
        ...
        while (!isSimulationDone && eventQueue.isNotEmpty) {
            ...
            exposeSubmissionTime += MILLIS_BETWEEN_ENV_EXPOSE
            eventQueue.submit(ExposeEnvironmentEvent(exposeSubmissionTime, environment))
        }
