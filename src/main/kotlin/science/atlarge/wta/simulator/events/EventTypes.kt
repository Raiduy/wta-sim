...
class ExposeEnvironmentEvent(time: Ticks, val env: Environment) : Event(time) {
    override val eventType: EventType
        get() = EventType.EXPOSE_ENVIRONMENT

    override fun toString(): String {
        return "ExposeEnvironmentEvent(time=$time, environment=${env.machines.first().instantEnergyConsumption})"
    }

    fun toOutFile(): String {
        var statsCollector = StringBuilder()
        for (machine in env.machines) {
            statsCollector.appendLine("$time,${machine.cluster.id}-${machine.id},${machine.instantEnergyConsumption}")
        }
        return statsCollector.toString()
        // return "$time\tenvironment=${env.machines.first().instantEnergyConsumption})"
    }
